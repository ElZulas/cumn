...
// Configuración de la conexión a MySQL remoto
const db = mysql.createConnection(configRemoto);

db.connect((err) => {
  if (err) {
    console.error('❌ Error al conectar a la base de datos remota:', err);
    console.error('💡 Verifica la configuración en config-remoto.js');
    return;
  }
  console.log('✅ Conectado a la base de datos MySQL remota');
  console.log(`🌐 Servidor: ${configRemoto.host}:${configRemoto.port}`);
...
app.listen(port, () => {
  console.log(`🚀 Servidor backend remoto escuchando en http://localhost:${port}`);
  console.log(`🌐 Conectado a: ${configRemoto.host}:${configRemoto.port}/${configRemoto.database}`);
}); 